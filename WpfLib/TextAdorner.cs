    ...
    where TShapeTag : ITextAdornable
{
    ...
    public static readonly Typeface TypeFace = new("Arial");
    protected override void OnRender(DrawingContext drawingContext)
    {
        if (AdornedElement is Shape shape /*&& shape.IsLabelUsed*/)
        {
            Rect segmentBounds = new(shape.DesiredSize);
            if (shape.Tag is TShapeTag shapeTag)
            {
                FormattedText ft = new(shapeTag.AdornmentText, Thread.CurrentThread.CurrentCulture, 
